...
if not st.session_state.get('authenticated', True): # Set to True for testing, change back to False for production
    st.warning("Please log in from the main page.")
    st.stop()
...
# ==========================================
# 1. Initialize session state for all 6 tabs
# ==========================================
if 'student_data' not in st.session_state:
    st.session_state.student_data = pd.DataFrame(columns=[
        "Student Name", "Father Name", "Email ID", "Phone Number", 
...
if 'admin_data' not in st.session_state:
    st.session_state.admin_data = pd.DataFrame(columns=[
        "Student Name", "Total Loan Requested", "Application Status"
    ])

# ==========================================
# Create 6 Tabs (Added Admin Tab)
# ==========================================
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    "Student Information", "Parents Information", "College Information", "KYC & Breakout", "Bank Selection", "Admin Dashboard"
])
...
with tab4:
    ...
    with st.form("kyc_form", clear_on_submit=True):
        ...
        if submit_kyc:
            ...
            # --- Auto-Populate Admin Dashboard ---
            # Try to grab the student's name from Tab 1 if they filled it out
            student_name = "Unknown Applicant"
            if not st.session_state.student_data.empty:
                student_name = st.session_state.student_data.iloc[-1]["Student Name"]
                
            new_admin_record = pd.DataFrame([{
                "Student Name": student_name,
                "Total Loan Requested": loan_amt_kyc,
                "Application Status": "Pending"
            }])
            st.session_state.admin_data = pd.concat([st.session_state.admin_data, new_admin_record], ignore_index=True)
            
            st.success("KYC documents uploaded and application sent to Admin for review!")
